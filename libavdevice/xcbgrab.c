 ...
 */
                                ...
                                int win_x, int win_y)
 {
     ...
     if (!cursor) { // Should free ci if cursor is NULL but ci is not
         free(ci);
         return;
     }
 ...
 static int xcbgrab_read_packet(AVFormatContext *s, AVPacket *pkt)
 {
     ...
     if (c->follow_mouse && p && p->same_screen) // Added p check
 ...
 #endif 
 #if CONFIG_LIBXCB_XFIXES
     if (ret >= 0 && c->draw_mouse && p && p->same_screen) // Added p check
                                ...
                                int            height)
 {
     ...
     if (  cudaMallocPitch((void **)&d_src, &d_srcPitch, srcPitch, height) ||
           cudaMallocPitch((void **)&d_y,   &d_pitchY,   pitchY,   height) ||
           cudaMallocPitch((void **)&d_u,   &d_pitchUV,  pitchUV,  height >> 1) ||
           cudaMallocPitch((void **)&d_v,   &d_pitchUV,  pitchUV,  height >> 1) )
         goto fail;
